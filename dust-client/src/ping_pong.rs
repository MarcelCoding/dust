...
impl PingPongHandler {
    pub fn new() -> Self {
        PingPongHandler{ ids: RwLock::new(HashMap::new()) }
    }

    pub async fn send_ping(&mut self, conn: &Mutex<Box<dyn Connection>>) -> anyhow::Result<u16> {
        ...
        conn.lock().await.send_pkg(Ping(PingPkgData::new(id))).await?;
